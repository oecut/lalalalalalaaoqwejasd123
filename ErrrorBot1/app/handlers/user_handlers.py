...
async def handle_stats_menu(callback: CallbackQuery):
    try:
        ...
        if stats_data:
            remaining_requests = max(0, settings.daily_request_limit - stats_data['user_today_total'])
            
            stats_text = f"""📊 <b>Ваша статистика</b>

📈 Общее количество запросов: <b>{stats_data.get('total_requests_all_time', 0)}</b>

💬 <b>Количество ответов ИИ</b>
<blockquote>• За все время: <b>{stats_data.get('total_text', 0)}</b>
• За сегодня: <b>{stats_data.get('user_today_total', 0)}</b></blockquote>

⚡ <b>Остаток лимитов</b>
<blockquote>• Запросов на сегодня: <b>{remaining_requests}</b></blockquote>

🌐 <b>Общая статистика бота</b>
<blockquote>• Активных пользователей: <b>{stats_data.get('active_users', 0)}</b>
• Всего групповых чатов: <b>{stats_data.get('total_groups', 0)}</b></blockquote>"""
        else:
            stats_text = "❌ Не удалось загрузить статистику. Возможно, вы еще не делали запросов."
    except Exception as e:
        logging.error(f"Ошибка получения пользовательской статистики: {e}")
        stats_text = "❌ Временная ошибка при загрузке статистики."
